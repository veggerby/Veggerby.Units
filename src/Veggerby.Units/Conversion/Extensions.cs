...
/// Only integral (int), double and decimal calculators are supported; other generic numeric types will raise
...
public static class Extensions
{
    ...
    /// </summary>
    /// <typeparam name="T">Underlying numeric type (currently int, double or decimal supported).</typeparam>
    ...
    public static Measurement<T> ConvertTo<T>(this Measurement<T> value, Unit unit) where T : IComparable
    {
        ...
        return PerformConversion(value, unit);
    }
    ...
    public static bool TryConvertTo<T>(this Measurement<T> value, Unit unit, out Measurement<T> result) where T : IComparable
    {
        ...
        try
        {
            result = PerformConversion(value, unit);
            return true;
        }
    ...
    /// <summary>
    /// Core conversion logic shared by <see cref="ConvertTo{T}"/> and <see cref="TryConvertTo{T}"/>.
    /// Preconditions: both arguments non-null, dimensions already verified to be compatible.
    /// </summary>
    private static Measurement<T> PerformConversion<T>(Measurement<T> value, Unit unit) where T : IComparable
    {
        if (value.Unit == unit)
        {
        ...
        return new Measurement<T>(newValue, unit, value.Calculator);
    }
}
