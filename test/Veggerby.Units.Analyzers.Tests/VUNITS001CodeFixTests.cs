...
public class VUNITS001CodeFixTests
{
    private static async Task<(string NewText, ImmutableArray<Diagnostic> Diagnostics)> RunAndFixAsync(string source)
    {
        ...
        var diag = diags.FirstOrDefault(d => d.Id == "VUNITS001");
        ...
        var fixProvider = new VUNITS001CodeFixProvider();
    ...
    [Fact]
    public async Task GivenMismatchedAddition_WhenFixApplied_ThenRightOperandConverted()
    {
        ...
        newText.Should().Contain("b.ConvertTo(a.Unit)");
    }
}